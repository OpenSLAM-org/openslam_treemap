...
void MainWindow::printStatistics()
{
  ...
  sprintf (txt, "n=%d landmarks, m=%d measurements, p=%d poses,  %d poses marginalized, %d poses sparsified, %d nodes, %d nodes to be optimized, memory:%5.1fMB (%4.1fKB/landmark): %9.6fs cost", 
           stat.n, stat.m, stat.p, stat.pMarginalized, stat.pSparsified, 
           stat2.nrOfNodes, stat2.nrOfNodesToBeOptimized,
           stat2.memory/1.0E6, stat2.memory/1.0E3/stat.n, TmNode::updateGaussianCost(3+2*stat.n));
  statusBar()->message (txt);  
}
...
void MainWindow::updateViews (int level)
{
  if (0<=treemapShown && treemapShown<(int) sim.treemap2D.size()) {
    treemapView->setTreemap (&sim.treemap2D[treemapShown]);    
    int myLevel = level;
    if (myLevel<0) myLevel = sim.sim.robotOldZ;
    mapView->setDotsFromTreemap (sim.treemap2D[treemapShown], myLevel);    
  }
  else {
...
void MainWindow::ensureRobotVisible ()
{
  if (sim.isRunning()) {    
    ...
    mapView->ensureVisible (x, y, 2*sim.sim.robot.maxDistance);  
  }  
}



void MainWindow::updateViewsAndMap (int level)
{
  updateViews (level);
  ensureRobotVisible ();  
}
...
void MainWindow::testSimulator ()
{
  try {    
     sim.sim.load ("../mzhlevel3quad100storiesC/mzhlevel3quad100stories.bui");
  
     FILE* f = fopen ("poses.dat", "w");
     FILE* f2= fopen ("landmarks.dat", "w");
     double x, y, theta;
     x = y = theta = 0;  
     while (!sim.sim.isFinished()) {
       VmVector3 odo;
       VmMatrix3x3 cov;    
       sim.sim.step (odo, cov);
       double c = cos(theta) , s = sin(theta);    
       x += c*odo[0] - s*odo[1];
       y += s*odo[0] + c*odo[1];
       theta += odo[2];    
       fprintf (f, "%f %f %f\n", x, y, 10.0*sim.sim.robotZ);
       vector<TmgSimulator::Observation> obs;    
       sim.sim.observe (obs);
       c = cos(theta);
       s = sin(theta);    
       for (int i=0; i<(int) obs.size(); i++) {
         double xx = x + c*obs[i].pos[0] - s*obs[i].pos[1];
         double yy = y + s*obs[i].pos[0] + c*obs[i].pos[1];
         fprintf (f2, "%f %f %f\n", xx, yy, 10.0*sim.sim.robotZ);      
       }
     }  
     fclose(f);  
     fclose(f2);
...
void MainWindow::runSLAM (const char* filename)
{
  try {
    ...
    sim.initTmgSimulator (filename);    
    setArea ();
    int ctr = 0;     
    while (!sim.sim.isFinished()) {
      SimulationContext::StatisticEntry stat;      
      sim.simStep (stat);  
      updateViewsAndMap ();
      char txt[1000];
